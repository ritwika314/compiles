...
\end{itemize}
The components of an agent configuration in the tuple $L_i$ are accessed using the usual dot ($\cdot$) notation. That is, $L_i.M$ refers to the local context $M$ in agent configuration $L_i$, for the agent with pid $i$ in the system. We use the notation $\pwl$ to represent the set of all possible values that the configuration of a single agent can have. 
...
We define the semantics of the overall system in terms of a nondeterministic, discrete-time transition system or a system automaton. The state of the automaton is defined in terms of {\em system configurations\/} consisting of the agent configurations and the state of the physical environment. 

A {\em system configuration\/} 
%with sampling parameter $\delta>0$ 
is a tuple $\gconfig = (\lset,{S},\tau,\turn)$, where
\begin{itemize}
	\item ${L} = \{\lconfig{i}\}_{i\in\UINS}$ is a list of {\em agent configurations\/}; $\lconfig{i}$ is the  configuration of the $i^{\mathit{th}}$ agent. $\pwl$ is the set of all possible configurations of all the agents in the system.
...
The global configuration changes according to state transition rules or {\em update rules\/}. The  update rules are rewriting rules that model the changes in the agent configurations due to execution of agent programs as well as the changes in the environment. Agent programs consist of events and events consist of sequence of $\lgname$ statements. Thus, in describing the transition rules, we start with statement level transitions. 
...
Execution of each statement in a $\lgname$ program corresponds to updating the 

\sayan{Stopping here.}
semantics are given by update rules of the type
...
\newline
Rule \textsc{Lvar-assign} describes the semantics of a local variable assignment. $x\notin \mathit{Keys}(S) \wedge x \in \mathit{Keys(L.M)}$ represents the fact that there is no mapping from $x$ in the global context $S$, but there is one in the local context $L.M$. The notation $\mathit{pc} = x = c \rightsquigarrow \mathit{pc}^\prime$ represents that syntactic production $\mathit{pc}^\prime$ is the one following the statement $x = c$ from the production $\mathit{pc}$. $M[x\mapsto v]$ refers to the fact that in the map $M$, the key $x$ maps to the value $v$. 
