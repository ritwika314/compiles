...
Brief introduction to CyPhyHouse~\cite{CyPhyhouse:arxiv19}.
Koord language, simulator.
Shared variable.
...
First, we illustrate how a complex distributed mapping application can be programmed using the   CyPhyHouse infrastructure~\cite{CyPhyhouse:arxiv19}. The application program \dmap is X lines of Koord code with a \sayan{handful} external functions. 
%
The application program, takes full advantage of the Koord language's features shared memory semantics. Individual robots update their local maps using LIDAR scans, and a \sayan{single line of Koord code} merges the local maps to update the shared global map. 
