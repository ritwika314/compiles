...
The overall constraint then becomes $$ \bigwedge_{i\in \UINS}\mathit{Pr} \wedge E \wedge A \wedge  I \Rightarrow I'$$.
...
\begin{assumption}
        \label{stlinemotion}
 There is an $\epsilon << d_s$, such that given a function $f' : [Point] \times [Point] \mapsto \mathbb{B}$  where $f'(x, y)$ is \emph{True} if $x$ and $y$ are at least $\epsilon$ distance apart, where $x$ and $y$ are either paths or positions,
    $$\neg M.reached_i \Rightarrow f'(M.pos_i, sp[i])$$
\end{assumption}

Clearly, $f'(x,y) \rightarrow f(x,y)$. From \asum{pathclear} and \asum{stlinemotion}, to prove \inv{inv:lineform}, we only need to prove the following invariant.
