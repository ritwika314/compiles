...
\iffalse
\rg{We provide the $\lgname$ definitions of all auxiliary library functions used in the code in the appendix \ref{auxfuncs}.}
...
\end{figure}
\fi
With the addition of an explicit neighbor relation, the \emph{{Squareform} app as shown in \reffig{squareform} can be used to generate a rhombus shape in the plane formed by four extremal points. The code defines a local variables $nbrs$ which is a static list of list of integers, where the $i$th element is a list of the $\mathid{pid}$s of the neighbors of the agent with $\mathit{pid}$ $i$. The list of $\mathit{pid}$s of the extremal points is stored in a list of integers called $\mathit{corners}$. If the agent $\mathit{pid}$ is not in $\mathit{corners}$ , then it moves to the mid point of all its neighbors.
