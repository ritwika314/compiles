...
Given a sound occupancy mapping $\map_i$, and $q^\prime \in \ff(\map_i)$, another sound mapping $\map_i^\prime$ can be constructed, such that $\map_i^\prime(q^\prime) = \world_Q(q^\prime)$, and $\forall q \neq q^\prime,\map_i(q) = \map_i^\prime(q)$. This follows from the definition of reachability and \lem{extension}. Taken in conjunction with our assumption on the initial positions of each robot, this leads to a strategy for computing sound occupancy mapping functions.
