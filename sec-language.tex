...
\newcommand{\SVarUpdate}{\textsc{SharedAssign}\xspace}
\newcommand{\LVarUpdate}{\textsc{LocalAssign}\xspace}
...
\inferrule*[leftskip=1cm, Right=\SVarUpdate]
    {
    x \in \mathit{Keys}({S}) \wedge x \in \mathit{Keys}(\lconfig{i} .{M}) \wedge \lconfig{i}^\prime.M = \lconfig{i}.M[x\mapsto v]
    }
    {\langle{S},\lconfig{i}, x = v \rangle {\rightarrow_{S}}  \langle{S}[x\mapsto v],\lconfig{i}^\prime,\cdot\rangle}\label{va1} \\
\inferrule*[leftskip=1cm, Right=\LVarUpdate]
    {
    x \notin \mathit{Keys}({S}) \wedge x \in \mathit{Keys}(\lconfig{i} .{M}) \wedge\  \lconfig{i}^\prime.M = \lconfig{i}.M[x\mapsto v]  }
    {\langle{S},\lconfig{i}, x = v \rangle {\rightarrow_{S}}  \langle{S},\lconfig{i}^\prime,\cdot\rangle}\label{va2}\\
\end{mathpar}
...
\end{mathpar}
\caption{Partial system level semantic rules for \lgname.}\label{fig:partial-semantics-global}
\end{figure}
