...
\subsection{Formal syntax and semantics}\label{sec:semantics}
...
\subsection{\lgname model for Distributed CPS}

\chiao{Avoid mentioning hybrid automaton}

The (distributed) system of robots written in \lgname can be viewed as a system of nondeterministic hybrid automata~\cite{TIOAmon, Mitra07PhD}.
Suppose we have $\NMAX$ robots in the system each executing the same \lgname application program,
\lgname middleware enforces a synchronous \emph{round-by-round} execution,
...
In the rest of this section, we present the formal semantics of the $\lgname$ programming language.
%
...
The detailed statement-level semantics of \lgname has been developed in the $\mathbb{K}$ framework.
The complete formalization of the semantics in \K is available from~\cite{} \sayan{webpage/github}.
