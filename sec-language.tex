...
As mentioned in \refsect{overview}, instances of the same \lgname program runs on a distributed collection of $\NMAX$ robots.
...
User can then optionally specify a sequence of statements to set the initial values of program variables~(rule~\emph{Init}).
...
The effect of an event is a sequence of statements~(rule~\emph{Effect}).
We skip the syntax rules for statements, expressions, data types, and functions due to the page limit.
Statements in \lgname resembles those in most imperative languages and includes
conditional statements, function calls, assignments, atomic blocks of statements for mutual exclusion, etc.
...
\begin{tabular}{lrl}
    ...
    Effect    & ::= & \mterm{eff} \delim Stmt\onemore                                   \\
              &     &                                                                   \\
    Stmt      & ::= & \mterm{atomic} Stmt\onemore \vbar \textellipsis
\end{tabular}
...
%
\marginpar{\scriptsize\sayan{set of all lists of statements? Lets at least say something about syntax of stmt in the previous section.}}
%
