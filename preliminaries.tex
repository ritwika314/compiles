...
The (distributed) system of robots  executing the \dmap application written in $\lgname$ can be viewed as a nondeterministic, state machine automaton~\cite{TIOAmon, Mitra07PhD}. The complete semantics of $\lgname$ has been developed in the $\mathbb{K}$ framework and will appear elsewhere. For the purpose of the analysis here, we briefly describe the formalization. 
...
\marginpar{\scriptsize\sayan{We will need to introduce the different types of variables, program varianbles, sensors, actuators.}}
