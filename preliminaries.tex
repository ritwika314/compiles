...
%
Given any function $f:A \rightarrow S$ and a subset $A' \subseteq A$, 
    the restriction of $f$ to $A'$, written as $f \lceil A'$, is the function $g:A' \rightarrow S$ such that for all $a \in A'$, $g(a) = f(a)$.
...
For each robot $i$, the program variable $\mathit{map}_i$ will ideally store a qunantized restriction of $\world$. That is, the type of this variable will be $\mathit{map}_i: Q' \rightarrow \mathbb{B}$, where $Q' \subseteq Q$. 

Now we can formally state the requirements of the mapping application.
\begin{enumerate}
	\item Always, each robot's map is a quantized restriction of the ground truth, that is, $\map_i: Q_i \mapsto \mathbb{B}$, where $Q_i\subseteq Q$ and 
	\item 
\end{enumerate}
