...
We will discuss the key features of the Koord language and programming system with an example. The Koord application shown in Figure~\ref{fig:lineform} is a simple  formation control protocol of the type often used for drone shows like the one seen in Figure~\ref{fig:firefly}. The code in Figure~\ref{fig:lineform} makes an arbitrary number of robots (drones) line up uniformly between two extremal robots. \sayan{Small modifications} to the code make the drones form other shapes like  squares, cubes, and stars. 
...
\sayan{introduce sensor and actuator ports using the LineForm example.}
...
\paragraph{Compiler / execution platform}
