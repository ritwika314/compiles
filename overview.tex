...
the controller implementing \emph{Motion} module indeed keeps the robot inside $\mathit{rect}(x_{min}, x_{max})$.
}
For this step, one has to reason about how each robot  moves when its implementation of the \emph{Motion} module is given a \emph{target}.
...
%$f$ is a function giving the set of positions of the robot at time $t$, moving to $M.tgt$, from $M.pos$.}.
...
\asum{lineform-assume} seems reasonable at the first glance, but may not be satisfied, for example, by nonholonomic dynamics of wheeled robots. 
